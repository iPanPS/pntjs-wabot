...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        try {
            ...
            const [userCoin, userPremium, userTicTacToe] = await Promise.all([
                global.db.get(`user.${senderNumber}.userCoin`) || "-",
                global.db.get(`user.${senderNumber}.isuserPremium`) ? "Ya" : "Tidak",
                global.db.get(`user.${senderNumber}.userTicTacToe`) || "-",
            ]);
            ...
            return await ctx.reply({
                ...
                caption: `${quote(`${await global.tools.msg.translate("Nama", userLanguage)}: ${ctx.sender.pushName}`)}\n` +
                    ...
                    `${quote(`${await global.tools.msg.translate("TicTacToe Trpohy", userLanguage)}: ${userTicTacToe}`)}\n` +
